  ...
  return (
    <nav className='relative flex items-center justify-between py-4 border-b border-zinc-200'>
      ...
      <div className='w-4/6 hidden lg:flex items-center justify-end'>
        ...
        {!isAuthenticated && (
          <Link
            className='ms-4 bg-black rounded px-4 py-2 text-zinc-100 hover:bg-blue-600 transition-all duration-300'
            to='/auth'
          >
            Log In
          </Link>
      ...
      {menuOpen && (
        <div className='absolute top-16 right-4 bg-white shadow-md rounded-lg p-4 flex flex-col space-y-3 lg:hidden'>
          {links.map((items, i) => (
            <Link
              key={i}
              to={items.to}
          ...
          {!isAuthenticated && (
            <Link
              to='/auth'
              className='bg-black rounded px-4 py-2 text-zinc-100 hover:bg-blue-600 transition-all duration-300'
              onClick={() => setMenuOpen(false)}
            >
              Log In
            </Link>
